...
metadata = {}
metadata['Tspec'] = 3
metadata['Ntimes'] = 256 #128
metadata['Nfreqs'] = 256 #127    # this MUST be one smaller than desired number of freq values
metadata['f_low'] = 300     #    as the total power is pushed into the bottom making 128
metadata['f_high'] = 10000
metadata['Nfft'] = 1024  #512
...
def getPkls(pklDir, year):
    pklfilelist = []
    try:
        # get list of the pkl files
        lst = os.listdir(pklDir)
        for fl in lst:
            if 'pkl' in fl and year in fl:
                pklfilelist.append(fl)
        pklfilelist.sort()        
    except:
        print("failed to load pkls from dir ", pklDir)
        exit()
    return pklfilelist
...
def compressPsdSliceLog(freqs, psds, flow, fhigh, nbands, doLogs):
    compressedSlice = np.zeros(nbands)  # totPwr in [0] and frequency of bands is flow -> fhigh in nBands steps
    ...
    totPwr = 0
    psdIdx1 = psdIdx2 = 0
    for i in range(1, nbands - 1):  # keep i=0 band for Total Power
        # find index of freqs just below fbands[i]
        while freqs[psdIdx1] < fbands[i]:
            psdIdx1 += 1
        psdIdx1 -= 1  # drop back to just below fband[i]
        # find index of freqs just above fbands[i+1]
        psdIdx2 = psdIdx1
        while freqs[psdIdx2] < fbands[i + 1]:
            psdIdx2 += 1
        psdIdx2 -= 1
        # find fraction of lowest freq psd that is INSIDE the band
        fracLow_in = min(df, (freqs[psdIdx1+1] - fbands[i]))/(freqs[psdIdx1+1] - freqs[psdIdx1])
        fracHigh_in = 0
        if psdIdx2  > psdIdx1:   # this means that the lower freq index + 1 didn't jump over the upper band boundary
            fracHigh_in = min(df, (fbands[i+1] - freqs[psdIdx2]))/(freqs[psdIdx2+1] - freqs[psdIdx2])
        # now add up the power in this band
        pwr = fracLow_in * (psds[psdIdx1] + psds[psdIdx1+1])/2
        pwr += fracHigh_in * (psds[psdIdx2] + psds[psdIdx2+1])/2
        if psdIdx2 > psdIdx1 + 1:  # now get the full psd bins and add them in
            for j in range(psdIdx2 - psdIdx1 - 1):
                pwr += (psds[psdIdx1 + 1 + j] + psds[psdIdx1 + 1 + j + 1])/2.0
        pwr *= df
        compressedSlice[i] = pwr
        totPwr += pwr
    #    print(i, pwr, totPwr)
    #  get the top band
    #   SET EQUAL TO HIGHEST BAND  -- should interpolate last bit of psd
    compressedSlice[-1] = compressedSlice[-2]
    #  put total power in the bottom band
    compressedSlice[0] = totPwr

    return compressedSlice

def compressPsdSliceLogOLDER(freqs, psds, flow, fhigh, nbands, doLogs):
